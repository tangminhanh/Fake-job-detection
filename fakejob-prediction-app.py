...
st.write("The app helps in knowing the likelihood of a posted job being fraudulent versus being non-fraudulent. Upon opening the app, the user is prompted to enter few details about the job ad, and based on the Logistic Regression model used, the user gets to know the likelihood of the job being fraudulent or non-fraudulent.")
...
logo = st.selectbox(
    ...
    ('Company logo: yes', 'Company logo: no'))
if logo == "Company logo: no":
...
trial = pd.DataFrame({'title':title,'department':department,'company_profile':company_profile,'description':description,'requirements':requirements,'benefits':benefits,'has_company_logo':logo,'employment_type':employment_type, "required_experience":required_experience, "required_education":required_education, "industry":industry, "function":function}, index=[0])
